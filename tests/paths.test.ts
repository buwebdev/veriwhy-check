...
test('user data roots follow documented platform conventions', () => {
  assert.equal(userDataRoot('darwin', { VERIWHY_CHECK_DATA_ROOT: '/custom/data' }, '/student'), resolve('/custom/data'));
...
test('browser cache respects an explicit Playwright location', () => {
  assert.equal(browserCacheRoot('linux', { PLAYWRIGHT_BROWSERS_PATH: '/managed/browser' }, '/student'), resolve('/managed/browser'));
