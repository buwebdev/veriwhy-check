...
const staticProfile: Profile = {
  id: 'sample/static', course: 'Sample', assignment: 'Static', version: '1.0.0',
...
test('static-only integration discovers a project and writes a local report', async () => {
  await withFixture('checker-run', async (root) => {
    ...
    const result = await checkProject('WEB-231/assignment-1.3', { searchPath: root, staticOnly: true, outputDirectory: join(root, 'report') });
...
test('missing and ambiguous projects become safe learner-facing errors', async () => {
  await withFixture('checker-errors', async (root) => {
    await assert.rejects(checkProject('WEB-231/assignment-1.3', { searchPath: root, staticOnly: true }), /No project matching/);
    for (const folder of ['one', 'two']) {
      await mkdir(join(root, folder));
      await writeFile(join(root, folder, 'project01-02.html'), '');
    }
    await assert.rejects(checkProject('WEB-231/assignment-1.3', { searchPath: root, staticOnly: true }), /Multiple projects/);
  });
});
...
test('dependency preparation reports a corrective failure instead of a process trace', async () => {
  await withFixture('checker-install', async (root) => {
    const profile: Profile = {
      id: 'sample/node', course: 'Sample', assignment: 'Node', version: '1.0.0',
      project: { kind: 'node', entry: 'index.js', install: 'npm-ci' },
      requirements: [{ id: 'run', label: 'Runs', rules: [{ kind: 'files', paths: ['index.js'] }] }]
    };
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: 'broken', dependencies: { missing: '1.0.0' } }));
...
test('Angular preparation injects the public check only into the prepared project', async () => {
  await withFixture('checker-angular', async (root) => {
    const profile: Profile = {
      id: 'WEB-425/lab-1.1', course: 'WEB 425 Angular with TypeScript', assignment: 'Lab', version: '1.0.0',
      project: { kind: 'npm', markers: ['package.json'], install: 'npm-ci' },
      requirements: [{ id: 'build', label: 'Builds', rules: [{ kind: 'files', paths: ['package.json'] }] }]
    };
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: 'empty', version: '1.0.0' }));
    await writeFile(join(root, 'package-lock.json'), JSON.stringify({ name: 'empty', version: '1.0.0', lockfileVersion: 3, packages: { '': { name: 'empty', version: '1.0.0' } } }));
    await prepareProject(root, profile);
    assert.match(await readFile(join(root, 'src', 'app', 'veriwhy-check.public.spec.ts'), 'utf8'), /Richard Krasso/);
  });
});
