...
test('profile parser rejects unknown keys, duplicate ids, and traversal', () => {
  assert.throws(
    () => parseProfileSource('WEB-330/assignment-1.3', validProfile.replace('version: 1.0.0', 'version: 1.0.0\nsecret: true')),
    /unsupported field/
  );
  assert.throws(
    () => parseProfileSource('WEB-330/assignment-1.3', validProfile.replace('id: timer-behavior', 'id: required-files')),
    /duplicate requirement/
  );
  assert.throws(
    () => parseProfileSource('WEB-330/assignment-1.3', validProfile.replace('project08-01.html', '../project08-01.html')),
    /relative paths/
  );
});
...
test('profile parser rejects arbitrary commands and invalid expressions', () => {
  ...
  assert.throws(() => parseProfileSource('WEB-330/assignment-1.3', unsafeCommand), /executable must be npm/);

  const sourceRule = validProfile.replace(
    'kind: browser\n        test: WEB-330/project8-1\n        case: countdown\n        timeoutSeconds: 20',
    "kind: source\n        roots: [.]\n        all: ['[invalid']"
  );
  assert.throws(() => parseProfileSource('WEB-330/assignment-1.3', sourceRule), /invalid regular expression/);
});

test('profile parser covers supported project and deterministic rule variants', () => {
  const nodeProfile = validProfile
    .replace('kind: static-web\n  entry: project08-01.html\n  locate: [project08-01_txt.html]', 'kind: node\n  entry: index.js\n  install: none')
    .replace('kind: browser\n        test: WEB-330/project8-1\n        case: countdown\n        timeoutSeconds: 20', 'kind: node-test\n        test: WEB-340/sample\n        case: run\n        timeoutSeconds: 20');
  assert.equal(parseProfileSource('WEB-330/assignment-1.3', nodeProfile).project.kind, 'node');

  const npmProfile = validProfile
    .replace('kind: static-web\n  entry: project08-01.html\n  locate: [project08-01_txt.html]', 'kind: npm\n  markers: [package.json, angular.json]\n  install: npm-ci')
    .replace('kind: browser\n        test: WEB-330/project8-1\n        case: countdown\n        timeoutSeconds: 20', 'kind: command\n        executable: npm\n        args: [run, build]\n        timeoutSeconds: 120');
...
test('profile parser explains malformed YAML and common schema mistakes', () => {
  ...
  assert.throws(() => parseProfileSource('WEB-330/other', validProfile), /does not match requested/);
  assert.throws(() => parseProfileSource('WEB-330/assignment-1.3', validProfile.replace('version: 1.0.0', 'version: first')), /semantic version/);
  assert.throws(() => parseProfileSource('WEB-330/assignment-1.3', validProfile.replace('timeoutSeconds: 20', 'timeoutSeconds: 0')), /integer from 1/);
  assert.throws(() => parseProfileSource('WEB-330/assignment-1.3', validProfile.replace('case: countdown', 'case: Not Valid')), /case must use lowercase/);
});

test('profile parser rejects empty, mistyped, and unsupported rule values', () => {
  const replaceBrowser = (replacement: string): string => validProfile.replace(
    'kind: browser\n        test: WEB-330/project8-1\n        case: countdown\n        timeoutSeconds: 20', replacement
  );
  const invalidSources = [
    ...
    validProfile.replace('kind: static-web\n  entry: project08-01.html\n  locate: [project08-01_txt.html]', 'kind: npm\n  markers: []\n  install: npm-ci'),
    validProfile.replace('kind: static-web\n  entry: project08-01.html\n  locate: [project08-01_txt.html]', 'kind: node\n  entry: index.js\n  install: always'),
    ...
    replaceBrowser('kind: command\n        executable: npm\n        args: []\n        timeoutSeconds: 20'),
    replaceBrowser('kind: unsupported'),
    validProfile.replace('id: timer-behavior', 'id: Invalid ID'),
    validProfile.replace('rules:\n      - kind: browser', 'rules: []\n      # removed rule\n      # - kind: browser')
  ];
  for (const source of invalidSources) assert.throws(() => parseProfileSource('WEB-330/assignment-1.3', source));
});
