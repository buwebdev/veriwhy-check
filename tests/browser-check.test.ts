 ...
 *
 * This suite proves browser isolation, traversal resistance, request blocking,
 * observable pass/fail behavior, and cleanup. Fixtures arrange only the state a
 * case needs; assertions document the security or functional invariant that a
...
test('browser runner identifies missing entry and check modules before launch', async () => {
  await withFixture('browser-missing', async (root) => {
    const missingEntry = await runBrowserCheck(
      root,
      'missing.html',
      'WEB-231/sample',
      'case',
    ...
    const missingCheck = await runBrowserCheck(
      root,
      'index.html',
      'WEB-231/sample',
      'case',
    ...
    assert.equal(
      cleanBrowserError(new Error(`${root}/index.html failed`), root),
      'selected project/index.html failed'
    );
  });
});
