...
import { isNewerVersion, normalizeVersion, releaseAssetName, updateApplication, type UpdateServices } from '../src/update.js';
...
test('up-to-date checks do not download or run an installer', async () => {
  ...
  const services: UpdateServices = {
    fetch: async () => response({ tag_name: 'v0.1.0', html_url: 'https://example.test', assets: [] }),
    run: async () => { runs += 1; return { passed: true, detail: '' }; }
  };
...
test('update failures preserve the current installation and explain why', async () => {
  const missing: UpdateServices = {
    fetch: async () => response({ tag_name: 'v9.0.0', html_url: 'https://example.test', assets: [] }),
...
test('unsigned, corrupted, and unpackable updates stop before activation', async () => {
  await withFixture('update-safety', async (root) => {
    ...
    const metadata = (digest: string | null) => response({
      tag_name: 'v9.0.0', html_url: 'https://example.test',
      assets: [{ name, browser_download_url: 'https://example.test/archive', digest }]
    });
    const run = async () => ({ passed: false, detail: 'Archive rejected.' });

    await assert.rejects(updateApplication({ fetch: async () => metadata(null), run }, {
    ...
    await assert.rejects(updateApplication({
      fetch: async () => ++request === 1 ? metadata(`sha256:${'0'.repeat(64)}`) : new Response(new Uint8Array([1, 2, 3])),
      run
    }, { currentVersion: '1.0.0', cacheDirectory: root, dataDirectory: root }), /failed its safety check/);
    ...
    await assert.rejects(updateApplication({
      ...
      run
    }, { currentVersion: '1.0.0', cacheDirectory: root, dataDirectory: root }), /could not be unpacked/);
  });
});
  ...
  await withFixture('update-record', async (root) => {
    ...
    const services: UpdateServices = {
      fetch: async () => ++request === 1
        ? response({ tag_name: 'v9.0.0', html_url: 'https://example.test', assets: [{ name, browser_download_url: 'https://example.test/archive', digest: `sha256:${digest}` }] })
        : new Response(bytes),
      run: async () => { run += 1; return { passed: true, detail: '' }; }
    };
    await assert.rejects(updateApplication(services, { currentVersion: '1.0.0', cacheDirectory: root, dataDirectory: join(root, 'missing') }), /installation record is missing/);
    assert.equal(run, 1);
  });
});
...
test('verified update archive installs beside the current version and switches the launcher', async () => {
  await withFixture('update-success', async (root) => {
    ...
    const packed = await runCommand('tar', ['-czf', archive, '-C', stage, 'install.mjs', 'payload'], root, 30);
    ...
    const services: UpdateServices = {
      fetch: async () => {
        request += 1;
        return request === 1
          ? response({ tag_name: 'v1.0.1', html_url: 'https://example.test', assets: [{ name: assetName, browser_download_url: 'https://example.test/archive', digest: `sha256:${digest}` }] })
          : new Response(new Uint8Array(bytes), { status: 200 });
      },
      run: runCommand
    };
    const result = await updateApplication(services, { currentVersion: '1.0.0', dataDirectory: data, cacheDirectory: join(root, 'cache'), platform: process.platform, architecture: process.arch });
    assert.equal(result.changed, true);
    assert.match(result.message, /updated from 1\.0\.0 to 1\.0\.1/);
    assert.equal(JSON.parse(await readFile(join(data, 'install.json'), 'utf8')).activeVersion, '1.0.1');
  });
});
