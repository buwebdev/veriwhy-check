/**
 * @file WEB 330 Assignment 6.2 reservation browser checks.
 * @author Richard Krasso
 *
 * The cases exercise reservation inputs, displayed confirmation, and disclosed
 * boundaries as observable browser behavior. They grade whether the application
 * works, not whether a student copied the reference solution's organization.
 */
...
// Each case owns one reservation behavior so the generated feedback can name an
// actionable requirement rather than report an undifferentiated suite failure.
export const cases = {
  ...
  async 'callback-delay'(page) {
    const result = await page.evaluate(
      async () =>
        await new Promise((resolve) => {
          ...
          reserveTable(
            2,
            (message) => resolve({ message, elapsed: performance.now() - started }),
            40
          );
        })
    );
    equal(
      result.message,
      'Table number 2 has been successfully reserved.',
      'Reservation callback message'
    );
    if (result.elapsed < 30)
      throw new Error('The available-table callback did not wait for the requested delay.');
  ...
  async 'available-and-reserved'(page) {
    ...
    await page.waitForFunction(() =>
      document.querySelector('#message')?.textContent.includes('successfully reserved')
    );
    equal(
      normalized(await page.locator('#message').textContent()),
      'Table number 1 has been successfully reserved.',
      'Available-table message'
    );
    await submit(page, 1);
    equal(
      normalized(await page.locator('#message').textContent()),
      'Table number 1 is already reserved.',
  ...
  async 'unknown-table'(page) {
    ...
    equal(
      normalized(await page.locator('#message').textContent()),
      'Table number 99 does not exist.',
