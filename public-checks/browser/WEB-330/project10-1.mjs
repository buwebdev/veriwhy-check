/**
 * @file WEB 330 Assignment 3.2 browser behavior checks.
 * @author Richard Krasso
 *
 * The cases verify the page's public interaction contract and visible results.
 * Checks intentionally accept equivalent implementations and report only
 * missing behavior, incorrect outcomes, or browser execution errors.
 */

import { equal, noBrowserErrors } from '../helpers.mjs';

// Named cases are public API between YAML and this module; their labels make the
// assessment-to-code relationship transparent to students and maintainers.
export const cases = {
  ...
  async 'pointer-drag'(page) {
    ...
    const before = await piece.evaluate((element) => ({
    ...
    await page.mouse.up();
    const after = await piece.evaluate((element) => ({
      left: element.style.left,
      top: element.style.top,
    ...
    if (after.left === before.left || after.top === before.top)
      throw new Error('Pointer movement did not change the puzzle piece position.');
    if (Number(after.zIndex) <= 1)
      throw new Error('Dragging did not raise the puzzle piece z-index.');
