...
export const cases = {
  async 'create-recipe'(root) {
    equal(await call(root, `recipes.createRecipe(['flour','sugar','butter'])`), 'Recipe created with ingredients: flour, sugar, butter', 'Recipe result');
  ...
  async 'module-exports'(root) {
    equal(await call(root, `Object.keys(recipes).sort()`), ['createRecipe', 'quit', 'setTimer'], 'Exported function names');
  ...
  async 'program-and-scripts'(root) {
    ...
    equal(result.stdout.trim().split(/\r?\n/), [
