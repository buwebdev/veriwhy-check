/**
 * @file WEB 340 Assignment 5.2 Node.js behavior checks.
 * @author Richard Krasso
 *
 * The named cases execute the student-facing contract with multiple values and
 * compare stable observable evidence. No case awards credit merely because a
 * source pattern resembles the instructor solution.
 */
...
// These stable identifiers are selected by YAML and keep every executable check
// traceable to a disclosed assignment requirement.
export const cases = {
  async 'successful-pies'(root) {
    equal(
      await bake(root, 'Apple', ['flour', 'sugar', 'butter', 'apples']),
      {
        value: 'Apple pie was successfully baked!',
        logs: [],
        exitCode: null
      },
      'Complete pie result'
    ...
    equal(
      await bake(root, 'Cherry', ['cherries', 'butter', 'flour', 'sugar', 'cinnamon']),
      {
        value: 'Cherry pie was successfully baked!',
        logs: [],
        exitCode: null
      },
  ...
  async 'missing-ingredients'(root) {
    ...
    equal(
      result.logs,
      ['Warning: Missing essential ingredients: flour, butter'],
