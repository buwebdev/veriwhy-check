...
export async function discoverGuidePages(sourceRoot: string): Promise<GuidePage[]> {
  if (!await pathExists(sourceRoot)) throw new Error(`Guide source folder not found: ${sourceRoot}.`);
  ...
  for (const filename of filenames) pages.push(describeGuidePage(filename, await readFile(join(sourceRoot, filename), 'utf8')));
  const duplicates = pages.filter((page, index) => pages.findIndex(({ slug }) => slug === page.slug) !== index);
  if (duplicates.length) throw new Error(`Guide pages produce duplicate names: ${duplicates.map(({ slug }) => slug).join(', ')}.`);
  return pages;
}
...
export async function renderGuidePage(page: GuidePage, pages: GuidePage[]): Promise<string> {
  const navigation = pages.map((candidate) => {
...
export async function buildDocsSite(
  ...
  for (const page of pages) await writeFile(join(outputRoot, `${page.slug}.html`), await renderGuidePage(page, pages), 'utf8');
  ...
  if (pages[0]!.slug !== 'index') await cp(join(outputRoot, `${pages[0]!.slug}.html`), join(outputRoot, 'index.html'));
  ...
  const index = pages.map(({ slug, title, plainText }) => ({ page: `${slug}.html`, title, text: plainText }));
  await writeFile(join(outputRoot, 'assets', 'search-index.js'), `/* Generated offline search index. Author: Richard Krasso. */\nwindow.VERIWHY_GUIDE_INDEX = ${JSON.stringify(index)};\n`, 'utf8');
