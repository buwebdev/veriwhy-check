...
export async function startStaticServer(root: string, entry: string): Promise<{ server: Server; origin: string }> {
  ...
  if (!address || typeof address === 'string') throw new Error('The local check server did not provide a port.');
...
export async function runBrowserCheck(
  ...
  if (!await pathExists(join(projectRoot, entry))) return { passed: false, detail: `Missing required entry page: ${entry}.` };
  const modulePath = join(checksRoot, `${test}.mjs`);
  if (!await pathExists(modulePath)) return { passed: false, detail: 'The selected public browser check is unavailable.' };

  let server: Server | undefined;
  let browser: Awaited<ReturnType<typeof chromium.launch>> | undefined;
  try {
    const loaded = await import(`${pathToFileURL(modulePath).href}?run=${Date.now()}`) as BrowserCheckModule;
    const selectedCase = loaded.cases?.[caseName];
    if (typeof selectedCase !== 'function') return { passed: false, detail: 'The selected public browser check case is unavailable.' };
    ...
    await context.route('**/*', async (route) => {
      ...
      if (url.startsWith(local.origin) || url.startsWith('data:') || url.startsWith('blob:')) await route.continue();
    ...
    return { passed: true, detail: typeof detail === 'string' && detail ? detail : 'Required browser behavior passed.' };
  } catch (error) {
