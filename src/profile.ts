...
function parseProject(value: unknown): ProjectConfig {
  ...
  if (kind === 'static-web') {
    ...
    const locate = value.locate === undefined ? undefined : safePaths(value.locate, 'project.locate');
    return {
      kind,
  ...
  if (kind === 'node') {
    ...
    if (install !== 'none' && install !== 'npm-ci') throw new Error('project.install must be none or npm-ci.');
    return { kind, entry: safePath(value.entry, 'project.entry'), install };
  }
  if (kind === 'npm') {
    assertKeys(value, ['kind', 'markers', 'install'], 'project');
    const markers = value.markers === undefined ? ['package.json'] : safePaths(value.markers, 'project.markers');
...
function behaviorRule(value: Record<string, unknown>, field: string, kind: 'browser' | 'node-test'): Rule {
  assertKeys(value, ['kind', 'test', 'case', 'timeoutSeconds'], field);
  const test = safePath(value.test, `${field}.test`);
  if (!/^[A-Za-z0-9._/-]+$/.test(test)) throw new Error(`${field}.test contains unsupported characters.`);
  const caseName = requiredString(value.case, `${field}.case`);
  if (!/^[a-z0-9][a-z0-9-]*$/.test(caseName)) throw new Error(`${field}.case must use lowercase letters, numbers, and hyphens.`);
  return { kind, test, case: caseName, timeoutSeconds: timeout(value.timeoutSeconds, `${field}.timeoutSeconds`, 120) };
}
...
function parseRule(value: unknown, field: string): Rule {
  ...
  if (kind === 'file-groups') {
    ...
    if (!Array.isArray(value.groups) || !value.groups.length) throw new Error(`${field}.groups must be a nonempty list.`);
    const groups = value.groups.map((group, index) => safePaths(group, `${field}.groups[${index}]`));
    if (groups.some((group) => !group.length)) throw new Error(`${field}.groups cannot contain an empty group.`);
    return { kind, groups };
  }
  if (kind === 'source' || kind === 'test-source') {
    ...
    if (!roots.length || !(all?.length || any?.length)) throw new Error(`${field} requires roots and at least one pattern.`);
  ...
  if (kind === 'test-count') {
    ...
    if (!Number.isInteger(value.minimum) || Number(value.minimum) < 0) throw new Error(`${field}.minimum must be a nonnegative integer.`);
  ...
  if (kind === 'command') {
    ...
    return { kind, executable: 'npm', args, timeoutSeconds: timeout(value.timeoutSeconds, `${field}.timeoutSeconds`, 600) };
  }
...
function parseRequirement(value: unknown, index: number): Requirement {
  ...
  if (!/^[a-z0-9][a-z0-9-]*$/.test(id)) throw new Error(`${field}.id must use lowercase letters, numbers, and hyphens.`);
  if (!Array.isArray(value.rules) || !value.rules.length) throw new Error(`${field}.rules must be a nonempty list.`);
  return {
    id,
...
function validateProfile(value: unknown, requestedId: string): Profile {
  ...
  assertKeys(value, ['id', 'course', 'assignment', 'version', 'project', 'requirements'], 'profile');
  const id = requiredString(value.id, 'profile.id');
  if (id !== requestedId) throw new Error(`Profile id ${id} does not match requested id ${requestedId}.`);
  const version = requiredString(value.version, 'profile.version');
  if (!/^\d+\.\d+\.\d+$/.test(version)) throw new Error('profile.version must use semantic version form, such as 1.0.0.');
  if (!Array.isArray(value.requirements) || !value.requirements.length) throw new Error('profile.requirements must be a nonempty list.');
  const requirements = value.requirements.map(parseRequirement);
  if (new Set(requirements.map(({ id: requirementId }) => requirementId)).size !== requirements.length) {
...
export function parseProfileSource(id: string, source: string): Profile {
  if (!/^[A-Za-z0-9._/-]+$/.test(id) || id.includes('..')) throw new Error(`Invalid profile id: ${id}.`);
  const document = parseDocument(source, { schema: 'core', strict: true, uniqueKeys: true });
  if (document.errors.length) {
    throw new Error(`Profile ${id} is invalid YAML: ${document.errors.map(({ message }) => message).join('; ')}`);
  }
...
export async function loadProfile(id: string): Promise<Profile> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') throw new Error(`Profile not found: ${id}.`);
    throw error;
  }
}
